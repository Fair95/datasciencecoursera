...
ind <- grep("comb+.+coal",SCC$Short.Name,ignore.case = TRUE)

Code<- SCC$SCC[ind]

datsub <- subset(NEI,SCC %in% Code)
year_sum <- with(datsub,tapply(Emissions,year,sum,rm.na = TRUE))
barplot(year_sum,main = "Total PM2.5 emission from coal combustion-related sources vs Year",xlab = "Year",ylab = "Total PM2.5 Emissions (tons)",col = "red")

dev.copy(png,file = "./wk4_4/plot4.png",height = 480,width = 480)
dev.off()