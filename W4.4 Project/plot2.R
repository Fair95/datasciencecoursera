...
datsub0 <- subset(NEI,fips == "24510")
year_sum <- with(datsub,tapply(Emissions,year,sum,rm.na = TRUE))

barplot(year_sum,main = "Total PM2.5 emission in Baltimore City vs Year",xlab = "Year",ylab = "Total PM2.5 Emissions (tons)",col = "red")

dev.copy(png,file = "./wk4_4/plot2.png",height = 480,width = 480)
dev.off()